...
def fill_form_and_take_screenshot(url, user_message, chat_id):
    try:
        with sync_playwright() as p:
            ...
            if message_field:
                ...
                page.keyboard.press("Enter")  # Имитируем нажатие клавиши Enter для отправки сообщения
            else:
                raise Exception("Поле 'Message' не найдено на странице.")
            
            # Бесконечный цикл для создания скриншотов каждые 0.3 секунды
            while True:
                screenshot_path = f"screenshot_{int(time.time())}.png"
                print(f"Создаю скриншот и сохраняю как: {screenshot_path}")
                page.screenshot(path=screenshot_path)
                
                # Отправляем скриншот в телеграм
                try:
                    with open(screenshot_path, 'rb') as screenshot_file:
                        bot.send_photo(chat_id, screenshot_file)
                except Exception as e:
                    print(f"Не удалось отправить скриншот: {e}")
                
                # Удаляем старый скриншот
                if os.path.exists(screenshot_path):
                    os.remove(screenshot_path)

                # Ждем 0.3 секунды перед созданием следующего скриншота
                time.sleep(0.3)
...
def send_welcome(message):
    bot.reply_to(message, "Привет! Отправь мне ссылку и текст, который ты хочешь вставить, и я начну присылать скриншоты сайта.")
...
def handle_message(message):
    ...
    if "," in message.text:
        ...
        if url.startswith("http://") or url.startswith("https://"):
            bot.reply_to(message, "Подожди, вставляю сообщение и начинаю присылать скриншоты сайта...")
            
            # Запускаем процесс скриншотов в отдельном потоке
            threading.Thread(target=fill_form_and_take_screenshot, args=(url, user_message, message.chat.id)).start()
        else:
